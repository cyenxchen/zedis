...
impl SearchMatcher {
    ...
    fn next_scroll_direction(&self) -> Option<MoveDirection> {
        if self.matched_ranges.is_empty() || self.current_match_ix >= self.matched_ranges.len().saturating_sub(1) {
            None
        } else {
    ...
    fn prev_scroll_direction(&self) -> Option<MoveDirection> {
        if self.matched_ranges.is_empty() || self.current_match_ix == 0 {
            None
        } else {
...
impl SearchPanel {
    ...
    fn prev(&mut self, _: &mut Window, cx: &mut Context<Self>) {
        let scroll_direction = self.matcher.prev_scroll_direction();
        if let Some(range) = self.matcher.next_back() {
            debug!(
                wrapped = scroll_direction.is_none(),
                current_match_ix = self.matcher.current_match_ix,
    ...
    fn next(&mut self, _: &mut Window, cx: &mut Context<Self>) {
        let scroll_direction = self.matcher.next_scroll_direction();
...
mod tests {
    ...
    #[test]
    fn test_search_wrap_scroll_direction() {
        let mut matcher = SearchMatcher::new();
        matcher.matched_ranges = Rc::new(vec![5..10, 15..20, 25..30]);

        matcher.current_match_ix = 0;
        assert!(matcher.prev_scroll_direction().is_none());
        assert!(matches!(matcher.next_scroll_direction(), Some(MoveDirection::Down)));

        matcher.current_match_ix = 1;
        assert!(matches!(matcher.prev_scroll_direction(), Some(MoveDirection::Up)));
        assert!(matches!(matcher.next_scroll_direction(), Some(MoveDirection::Down)));

        matcher.current_match_ix = 2;
        assert!(matches!(matcher.prev_scroll_direction(), Some(MoveDirection::Up)));
        assert!(matcher.next_scroll_direction().is_none());
    }
}
